export const API = {
  ...
  getRegisterFields: (merchantUuid: string): string => `/accounts/register-fields/${merchantUuid}`,
  ...
  getWatchHistoryForItem: (id: string): string => `/v2/accounts/media/watch-history/${id}`,
  ...
  getCloudfrontURL: (id: number, videoUrl: string): string => `/items/${id}/access/cloudfront?url=${videoUrl}`,
  ...
  // Branding
  getBranding: (clientId: string, brandingId: string | number): string => `/branding/paywall/${clientId}/${brandingId}`,

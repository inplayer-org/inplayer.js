export const API = (config: any) => ({
  ...
  getPurchaseHistory: (status: any, page = 0, size = 5) => `${config.BASE_URL}/items/access/customers?status=${status}&page=${page}&size=${size}`,
  ...
  getCloudfrontURL: (assetId: any, videoUrl: any) => `${config.BASE_URL}/items/${assetId}/access/cloudfront?url=${videoUrl}`,
  // Payment
  ...
  getBranding: (merchantUuid: any, brandingId: any) => `${config.BASE_URL}/branding/paywall/${merchantUuid}/${brandingId}`,

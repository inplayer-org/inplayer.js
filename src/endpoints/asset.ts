...
export const asset = (config: ApiConfig, request: Request) => {
   ...
   * @returns  {AxiosResponse<GetItemAccessV1>}
   */
  const checkAccessForAsset = async (id: number) => {
    return request.authenticatedGet(API.checkAccessForAsset(id), {
      headers: {
   ...
   * @returns  {AxiosResponse<boolean>}
   */
  const isFreeTrialUsed = async(id: number) => {
    return request.authenticatedGet(API.checkFreeTrial(id), {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
   ...
   * @returns  {AxiosResponse<ItemDetailsV1>}
   */
  const getAsset = async(assetId: number, merchantUuid: string) => {
    return request.get(API.getAsset(assetId, merchantUuid));
  }
  ...
  const getPackage = async (id: number) => {
    return request.get(API.getPackage(id));
  }
   ...
   * @returns  {AxiosResponse<GetAccessFee>}
   */
  const getAssetAccessFees = async (id: number) => {
    return request.get(API.getAssetAccessFees(id));
  }
  ...
  const getAssetsHistory = async (
    ...
    return request.authenticatedGet(
      API.getAssetsHistory(size, page, startDate, endDate),
      {
        headers: {
          Authorization: `Bearer ${request.getToken().token}`,
   ...
   * @returns  {Object}
   */
  const requestCodeAccess = async(codeAccessData: CodeAccessData) => {
    const formData = new FormData();
    ...
    localStorage.setItem(
      config.INPLAYER_ACCESS_CODE_NAME(codeAccessData.item_id),
      JSON.stringify(accessCode),
    );
   ...
   * @returns  {CodeAccessData | null}
   */
  const getAccessCode = (assetId: number) => {
    const accessCode = localStorage.getItem(config.INPLAYER_ACCESS_CODE_NAME(assetId),);
   ...
   * @returns  {Object}
   */
  const releaseAccessCode = async (assetId: number) => {
    const accessCode: CodeAccessData = getAccessCode(assetId);
    ...
    localStorage.removeItem(config.INPLAYER_ACCESS_CODE_NAME(assetId));
   ...
   * }
   */
  const getCloudfrontURL = async (assetId: number, videoUrl: string) => {
    return request.get(API.getCloudfrontURL(assetId, videoUrl), {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
  }
  return {checkAccessForAsset, isFreeTrialUsed, getAsset, getExternalAsset, getPackage,releaseAccessCode, getAssetAccessFees, getAssetsHistory, getCloudfrontURL};
}
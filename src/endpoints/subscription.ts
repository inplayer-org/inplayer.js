...
export const subscriptions = (config: ApiConfig, request: Request) => {
    /**
   * Gets all subscriptions for a given user
   * @method getSubscriptions
   * @async
   ...
   * @returns  {AxiosResponse<GetSubscription>}
   */
  const getSubscriptions = async (page = 0, limit = 15, status = '') => {
    return request.authenticatedGet(API.getSubscriptions(limit, page, status), {
      headers: {
  ...
  const getSubscription = async(id: number) => {
    return request.authenticatedGet(API.getSubscription(id), {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
   ...
   * @returns  {AxiosResponse<CancelSubscription>}
   */
  const cancelSubscription = async (unsubscribeUrl: string) => {
  ...
  return {getSubscriptions, getSubscription, cancelSubscription, createSubscription};
}
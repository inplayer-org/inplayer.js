...
export const payment = (config: ApiConfig, request: Request) => {
   /**
   * Get all payment methods for a User
   * @method getPaymentMethods
   * @async
  ...
  const getPaymentMethods = async () => {
    return request.authenticatedGet(API.getPaymentMethods, {
      headers: {
   ...
   * @returns  {AxiosResponse<any>}
   */
  const getPaymentTools = async (paymentMethodId: number) => {
  ...
  const createPayment = async (data: CreatePaymentData) => {
    ...
    return request.authenticatedPost(
      API.payForAsset,
      qs.stringify(body),
      {
        headers: {
   ...
   * @returns  {AxiosResponse<GeneratePayPalParameters>}
   */
  const getPayPalParams = async (data: PaypalParamsData) => {
    const formData = new FormData();
    ...
    return request.authenticatedPost(API.getPayPalParams, formData, {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
   ...
   * @returns  {AxiosResponse<PurchaseHistoryCollection[]>}
   */
  const getPurchaseHistory = async (status = 'active', page: number, limit: number) => {
    return request.authenticatedGet(
      API.getPurchaseHistory(status, page, limit),
      {
        headers: {
  ...
  const getDefaultCreditCard = async () => {
    return request.authenticatedGet(API.getDefaultCreditCard, {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
   ...
   * @returns  {<AxiosResponse<SetDefaultCard>}
   */
  const setDefaultCreditCard = async (data: SetDefaultCardPerCurrencyData) => {
    const body = {
      number: data.cardNumber,
      card_name: data.cardName,
      cvv: data.cvc,
    ...
    return request.authenticatedPut(
      API.setDefaultCreditCard,
      qs.stringify(body),
      {
        headers: {
   ...
   * }
   */
  const getDirectDebitMandate = async () => {
    return request.authenticatedGet(API.getDirectDebitMandate, {
      headers: {
        Authorization: `Bearer ${request.getToken().token}`,
      },
    });
   ...
   *  }
   */
  const createDirectDebitMandate = async (data: CreateDirectDebitMandateData) => {
    const body = {
      name: data.name,
      iban: data.iban,
    };
   ...
   *
   */
  const directDebitCharge = async (data: DirectDebitData) => {
    const body = {
      access_fee_id: data.accessFeeId,
      item_id: data.assetId,
      voucher_code: data.voucherCode,
   ...
   *  }
   */
  const directDebitSubscribe = async (data: DirectDebitData) => {
    const body = {
      item_id: data.assetId,
      access_fee_id: data.accessFeeId,
      voucher_code: data.voucherCode,
  ...
  return {getPaymentMethods, getPaymentTools, createPayment, confirmPayment, getPayPalParams, getPurchaseHistory, getDefaultCreditCard, getDirectDebitMandate, createDirectDebitMandate, setDefaultCreditCard, directDebitCharge, directDebitSubscribe};
}
